...
class MyRoom extends colyseus_1.Room {
    ...
    onCreate(options) {
        ...
        // set-up the game!
        this.setUp();
        this.onMessage("touch-block", (client, atIndex) => {
    ...
    setUp() {
        ...
        // create first block
        this.state.blocks.push(new MyRoomState_1.Block().assign({
            x: 8,
            y: 1,
            z: 8,
        }));
    ...
    createBlock(atIndex) {
        ...
        const block = new MyRoomState_1.Block();
        //
        ...
        } while (Math.abs(block.x - previousBlock.x) < 1 ||
        ...
        } while (Math.abs(block.z - previousBlock.z) < 1 ||
            (previousBlock2 && Math.abs(block.z - previousBlock2.z) < 1));
        this.state.blocks.push(block);
    }
    _applyBoundaries(coord) {
